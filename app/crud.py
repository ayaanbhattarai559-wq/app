...
def create_product(db: Session, data: schemas.ProductCreate) -> models.Product:
    ...
    try:
        ...
        db.rollback()
        raise DuplicateSKU(product.sku)
    db.refresh(product)
...
def update_product(db: Session, product_id: str, data: schemas.ProductUpdate) -> models.Product:
    ...
    try:
        ...
        db.rollback()
        raise DuplicateSKU(data.sku or product.sku)
    return get_product(db, product_id)


# Autogenerated SKUs look like KS-1000, KS-1001, ... -- easy to read, and
# always one higher than the highest KS-#### SKU currently in use, so two
# clicks of "Auto-Gen" can never hand out the same code. (If the
# highest-numbered product is later deleted, the next generated SKU could in
# theory reuse its old number -- harmless for a shop this size, since SKUs
# aren't reused while the product they belong to still exists.)
_AUTO_SKU_PREFIX = "KS-"
_AUTO_SKU_RE = re.compile(rf"^{re.escape(_AUTO_SKU_PREFIX)}(\d+)$")


def next_sku(db: Session) -> str:
    skus = db.execute(select(models.Product.sku)).scalars().all()
    highest = 999
    for sku in skus:
        m = _AUTO_SKU_RE.match(sku or "")
        if m:
            highest = max(highest, int(m.group(1)))
    return f"{_AUTO_SKU_PREFIX}{highest + 1}"
...
def delete_sale(db: Session, transaction_id: str) -> None:
    """Removes a sale record (and its line items) entirely -- e.g. to clean
    up test data. Does NOT restore stock or lower a product's sold count;
    those are separate, deliberate actions (see restock_variant/adjust_stock)."""
    tx = db.get(models.SaleTransaction, transaction_id)
    if not tx:
        raise NotFound(f"Sale {transaction_id} not found")
    db.delete(tx)
    db.commit()
...
def checkout(db: Session, data: schemas.CheckoutRequest) -> models.SaleTransaction:
    ...
    for line in data.items:
        ...
        effective_price = round(list_price * (1 - discount_percent / 100), 2)

        line_subtotal = effective_price * line.quantity
        ...
        tx.items.append(models.SaleTransactionItem(
            product_id=product.id, product_name=product.name,
            sku=variant.sku or product.sku, color=variant.color, size=variant.size,
            quantity=line.quantity, unit_price=effective_price, unit_cost=float(product.cost),
            subtotal=line_subtotal, profit=line_subtotal - line_cost,
            list_price=list_price, discount_percent=discount_percent,
        ))
